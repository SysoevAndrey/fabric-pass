...
The `contributors` table (`migrations/001_contributors.sql`, reshaped by `migrations/002_contributor_name_and_nullable_fields.sql`, `migrations/003_telegram_id_as_text.sql`, `migrations/004_provider_profile_fields.sql`, `migrations/005_contributor_status.sql`, `migrations/006_alias_and_agent_fields.sql`, `migrations/007_email_confirmation.sql`, `migrations/008_linkedin_fields.sql`, `migrations/009_admin_role.sql`, and `migrations/011_blocked_status.sql`):
...
- **Confirm** sets `status` to `'confirmed'`; **Block** sets it to `'blocked'` — both write directly (`src/lib/contributors.ts`'s `setContributorStatus`), the one place in this app that writes `status` outside the registry sync above. The write folds back through the registry file on the next scheduled export, same as any other DB-owned column — the narrow risk is a registry-file edit landing in the gap between an Admin's click and that next export, which would overwrite the in-app change back on the import that follows.
