...
The `contributors` table (`migrations/001_contributors.sql`, reshaped by `migrations/002_contributor_name_and_nullable_fields.sql`, `migrations/003_telegram_id_as_text.sql`, `migrations/004_provider_profile_fields.sql`, `migrations/005_contributor_status.sql`, `migrations/006_alias_and_agent_fields.sql`, `migrations/007_email_confirmation.sql`, and `migrations/008_linkedin_fields.sql`):
...
| `LINKEDIN_CLIENT_ID`, `LINKEDIN_CLIENT_SECRET` | **Optional** — from the LinkedIn application, see [LinkedIn](#linkedin) below. This app's only optional *provider*: with both unset, the app still boots and runs, the LinkedIn row is left off the profile form, and `/auth/linkedin` (and its callback) respond 404 |

The eleven above `RESEND_*` are required, not just for running the app: `src/lib/env.ts` validates the whole environment at import, and `next build` imports every route module while collecting page data, so `pnpm build` fails before it reaches any provider if even one variable is unset. Placeholder values satisfy this — the build never contacts a provider. `RESEND_*` and `LINKEDIN_CLIENT_ID`/`LINKEDIN_CLIENT_SECRET` are the exceptions, deliberately: this app needs to keep booting in an environment where email, or LinkedIn, hasn't been configured yet.
