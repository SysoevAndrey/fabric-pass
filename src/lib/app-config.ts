...
export async function syncAppConfig(config: AppConfigSync): Promise<void> {
  await pool.query(
    `INSERT INTO app_config (id, github_organization, discord_guild_id, discord_invite_url, updated_at)
     ...
     ON CONFLICT (id) DO UPDATE
       SET github_organization = EXCLUDED.github_organization,
           ...
           updated_at = now()`,
    [config.githubOrganization ?? null, config.discordGuildId ?? null, config.discordInviteUrl ?? null],
  )
}
